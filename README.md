...
`src/mazegen`は別プロジェクトでも再利用できるように分離するため、A-Maze-ing本体とディレクトリを分けて実装する。
...
- `a_maze_ing.py`: 処理をを統括する課題指定のエントリーポイント
- `config.txt`: 課題で求められているデフォルト設定ファイル
- `amazeing/parse_config.py`: 設定ファイルの解析
- `amazeing/output.py`: 課題指定の16進数形式へ変換し、ファイルを保存する処理
- `amazeing/display.py`: 迷路のビジュアライズと操作
- `src/mazegen/generator.py`: `MazeGenerator`クラスを含む、再利用可能な迷路生成モジュール
...
- `mazegen-<version>-py3-none-any.whl`: ビルド済みの再利用パッケージ。`.tar.gz`形式を使用することもできる
